...
# Corrected Roulette wheel - without elitism
...
class Individual:
    ...
    def create_gene(self):  # gives gene to the individual - i.e. route
        ...
        i = 1
        while i < n:
            possible_vertices = np.nonzero(theMap[self.route[
                i - 1]])  # should be accessed as possible_vertices[0], because its 0th element consists of the tuple
            while True:
                proposed_ver_index = random.randint(0,
                                                    len(possible_vertices) + 1)  # proposed index of possible_vertices
    ...
    def find_fitness(self):                 # 1/distance: more distance, lesser fitness
        self.fitness = round(1/self.distance, 2)
    ...
    def genetic_algorithm(self):
        ...
        for i in range(0, no_of_iterations):  # 1. run GA for given no of iterations
            ...
            new_population = Population(
                self.size)  # !!!IMP: Natural Selection method in Population class, to be used while this new population creation
            ...
            for crossoverIteration in range(0, no_of_couples):
                ...
                new_population.populationList.append(child1)
                new_population.populationList.append(child2)
                ...
                new_population.populationList.append(parent01)  # Step 4. Elitism
                new_population.populationList.append(parent02)

            self.mutation_on_population(mutationProbability)  # Step 5. Mutation

            # add randomly in remaining places - if remained
            while len(new_population.populationList) < self.size:
                inv = Individual()
                inv.create_new_member()
                new_population.populationList.append(inv)

            self.current_population.clear_population()
            self.current_population.populationList = copy.deepcopy(new_population.populationList)
            print('-' * 20)
